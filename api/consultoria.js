...
module.exports = async (req, res) => {
  ...
  if (nome.length < 3 || empresa.length < 2 || cnpj.replace(/\D/g, '').length !== 14 || !/^[^\s@]+@[^\s@]+\.[a-z]{2,}$/i.test(email) || tel.replace(/\D/g, '').length < 10 || body.lgpd !== true) return res.status(400).json({ ok: false, error: 'invalid' });
  if (!process.env.RESEND_API_KEY) return res.status(500).json({ ok: false, error: 'not_configured' });
  const html = `<h2>Novo pedido de consultoria — Reforma Tributária</h2><p><strong>Nome:</strong> ${esc(nome)}</p><p><strong>Empresa:</strong> ${esc(empresa)}</p><p><strong>CNPJ:</strong> ${esc(cnpj)}</p><p><strong>E-mail:</strong> ${esc(email)}</p><p><strong>Telefone:</strong> ${esc(tel)}</p><p><strong>Mensagem:</strong><br>${esc(mensagem).replace(/\n/g, '<br>')}</p><p><small>Consentimento LGPD aceito.</small></p>`;
